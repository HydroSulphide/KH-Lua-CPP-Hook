...
void print_message_w(const std::wstring &text, MessageType message_type, const std::string &mod_name) {
	HANDLE _console_handle = GetStdHandle(STD_OUTPUT_HANDLE);

	DWORD written; // Variable to store the number of characters written
	...
	SetConsoleTextAttribute(_console_handle, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	WriteConsoleW(_console_handle, text.c_str(), static_cast<DWORD>(text.length()), &written, NULL);
}

void print_message_line_w(const std::wstring &text, MessageType message_type, const std::string &mod_name) {
	print_message_w(text, message_type, mod_name);
	std::cout << std::endl;
}
...
__declspec(dllexport) void print_mod_message_w(const wchar_t *text, MessageType message_type, const char *mod_name) {
	print_message_w(text, message_type, mod_name);
}

extern "C" __declspec(dllexport) void print_mod_message_line_w(const wchar_t *text, MessageType message_type, const char *mod_name) {
	print_message_line_w(text, message_type, mod_name);
}
