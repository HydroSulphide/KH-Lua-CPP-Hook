...
void LuaBackend::LoadScripts(const std::vector<fs::path>& ScriptPaths,
  ...
  for (auto scriptsDir : ScriptPaths) {
    ...
    for (auto _path : fs::directory_iterator(scriptsDir)) {
      ...
      _script->luaState["GAME_ID"] =
          CRC::Calculate(_pathExe.c_str(), _pathExe.length(), CRC::CRC_32());
        ...
        if (_result.valid()) {
          if (!_script->initFunction && !_script->frameFunction) {
            ConsoleLib::MessageOutput(
                ...
                "errors.\n",
                3);
            ConsoleLib::MessageOutput("Initialization of this script cannot "
                                      "continue...\n",
                                      3);
            return;
          }
...
void LuaBackend::SetFunctions(LuaState* _state) {
  _state->set_function("ReadByte", sol::overload(
    [](std::uint64_t Address) -> std::uint8_t {
    return MemoryLib::ReadByte(Address, false);
    },
    [](std::uint64_t Address, bool absolute) -> std::uint8_t {
    return MemoryLib::ReadByte(Address, absolute);
    }));

  _state->set_function("ReadShort", sol::overload(
    [](std::uint64_t Address) -> std::uint16_t {
    return MemoryLib::ReadShort(Address, false);
    },
    [](std::uint64_t Address, bool absolute) -> std::uint16_t {
    return MemoryLib::ReadShort(Address, absolute);
    }));

  _state->set_function("ReadInt", sol::overload(
    [](std::uint64_t Address) -> std::uint32_t {
    return MemoryLib::ReadInt(Address, false);
    },
    [](std::uint64_t Address, bool absolute) -> std::uint32_t {
    return MemoryLib::ReadInt(Address, absolute);
    }));

  _state->set_function("ReadLong",sol::overload(
    [](std::uint64_t Address) -> std::uint64_t {
    return MemoryLib::ReadLong(Address, false);
    },
    [](std::uint64_t Address, bool absolute) -> std::uint64_t {
    return MemoryLib::ReadLong(Address, absolute);
    }));

  _state->set_function("ReadFloat", sol::overload(
    [](std::uint64_t Address) -> float {
        return MemoryLib::ReadFloat(Address, false);
    },
    [](std::uint64_t Address, bool absolute) -> float {
        return MemoryLib::ReadFloat(Address, absolute);
    }));

  _state->set_function("ReadBoolean", sol::overload(
    [](std::uint64_t Address) -> bool {
        return MemoryLib::ReadBool(Address, false);
    },
    [](std::uint64_t Address, bool absolute) -> bool {
        return MemoryLib::ReadBool(Address, absolute);
    }));

  _state->set_function("ReadArray", sol::overload(
    [](std::uint64_t Address, int Length) -> std::vector<std::uint8_t> {
        return MemoryLib::ReadBytes(Address, Length, false);
    },
    [](std::uint64_t Address, int Length, bool absolute) -> std::vector<std::uint8_t> {
        return MemoryLib::ReadBytes(Address, Length, absolute);
    }));

  _state->set_function("ReadString", sol::overload(
    [](std::uint64_t Address, int Length) -> std::string {
        return MemoryLib::ReadString(Address, Length, false);
    },
    [](std::uint64_t Address, int Length, bool absolute) -> std::string {
        return MemoryLib::ReadString(Address, Length, absolute);
    }));

  _state->set_function("WriteByte", sol::overload(
    [](std::uint64_t Address, std::uint8_t Input) {
        MemoryLib::WriteByte(Address, Input, false);
    },
    [](std::uint64_t Address, std::uint8_t Input, bool absolute) {
        MemoryLib::WriteByte(Address, Input, absolute);
    }));

  _state->set_function("WriteShort", sol::overload(
    [](std::uint64_t Address, std::uint16_t Input) {
        MemoryLib::WriteShort(Address, Input, false);
    },
    [](std::uint64_t Address, std::uint16_t Input, bool absolute) {
        MemoryLib::WriteShort(Address, Input, absolute);
    }));

  _state->set_function("WriteInt", sol::overload(
    [](std::uint64_t Address, std::uint32_t Input) {
        MemoryLib::WriteInt(Address, Input, false);
    },
    [](std::uint64_t Address, std::uint32_t Input, bool absolute) {
        MemoryLib::WriteInt(Address, Input, absolute);
    }));

  _state->set_function("WriteLong", sol::overload(
    [](std::uint64_t Address, std::uint64_t Input) {
        MemoryLib::WriteLong(Address, Input, false);
    },
    [](std::uint64_t Address, std::uint64_t Input, bool absolute) {
        MemoryLib::WriteLong(Address, Input, absolute);
    }));

  _state->set_function("WriteFloat", sol::overload(
    [](std::uint64_t Address, float Input) {
        MemoryLib::WriteFloat(Address, Input, false);
    },
    [](std::uint64_t Address, float Input, bool absolute) {
        MemoryLib::WriteFloat(Address, Input, absolute);
    }));

  _state->set_function("WriteBoolean", sol::overload(
    [](std::uint64_t Address, bool Input) {
        MemoryLib::WriteBool(Address, Input, false);
    },
    [](std::uint64_t Address, bool Input, bool absolute) {
        MemoryLib::WriteBool(Address, Input, absolute);
    }));

  _state->set_function("WriteArray", sol::overload(
    [](std::uint64_t Address, const std::vector<std::uint8_t>& Input) {
        MemoryLib::WriteBytes(Address, Input, false);
    },
    [](std::uint64_t Address, const std::vector<std::uint8_t>& Input, bool absolute) {
        MemoryLib::WriteBytes(Address, Input, absolute);
    }));

  _state->set_function("WriteString", sol::overload(
    [](std::uint64_t Address, const std::string& Input) {
        MemoryLib::WriteString(Address, Input, false);
    },
    [](std::uint64_t Address, const std::string& Input, bool absolute) {
        MemoryLib::WriteString(Address, Input, absolute);
    }));

  _state->set_function("WriteExec", 
    [](std::uint64_t Address, const std::vector<std::uint8_t>& Input) {
        MemoryLib::WriteExec(Address, Input);
    });

  _state->set_function("GetPointer",sol::overload(
    [](std::uint64_t Address) -> std::uint64_t {
        return MemoryLib::GetPointer(Address, 0, false); // Offset is defaulted to 0
    },
    [](std::uint64_t Address, std::uint64_t Offset,
        bool absolute) -> std::uint64_t {
        return MemoryLib::GetPointer(Address, Offset, absolute);
    }));
