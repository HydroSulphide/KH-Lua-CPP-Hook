...
class MemoryLib {
  private:
	...
	class protect_lock {
	  ...
	  public:
		protect_lock(std::uintptr_t address, std::size_t size) : address(address), size(size), protection(0), is_acquired(false) {
  ...
  public:
		...
		if (EnumProcessModules(InputHandle, hMods, sizeof(hMods), &cbNeeded)) {
			for (i = 0; i < (cbNeeded / sizeof(HMODULE)); i++) {
				...
				if (wil::GetModuleFileNameExW(InputHandle, hMods[i], szModName)) {
					if (szModName.find(InputName) != std::wstring::npos)
						return hMods[i];
				}
			}
		}
	...
	static int ExecuteProcess(std::wstring InputName, std::uint64_t InputAddress, bool InputEndian) {
		...
		if (CreateProcessW(InputName.c_str(), NULL, NULL, NULL, TRUE, 0, NULL, NULL, &_sInfo, &_pInfo) == 0)
			return -1;
	...
	template <typename T, std::enable_if_t<std::is_trivially_constructible_v<T>, int> = 0> static T readScalarAbsolute(std::uint64_t Address) {
	...
	static inline std::uint8_t ReadByteAbsolute(std::uint64_t Address) { return readScalarAbsolute<std::uint8_t>(Address); }

	static inline std::uint16_t ReadShortAbsolute(std::uint64_t Address) { return readScalarAbsolute<std::uint16_t>(Address); }

	static inline std::uint32_t ReadIntAbsolute(std::uint64_t Address) { return readScalarAbsolute<std::uint32_t>(Address); }

	static inline std::uint64_t ReadLongAbsolute(std::uint64_t Address) { return readScalarAbsolute<std::uint64_t>(Address); }
	...
	template <typename T> static T readScalar(std::uint64_t Address, bool absolute = false) {
		if (absolute) {
	...
	static inline std::uint8_t ReadByte(std::uint64_t Address, bool absolute = false) { return readScalar<std::uint8_t>(Address, absolute); }

	static inline std::uint16_t ReadShort(std::uint64_t Address, bool absolute = false) { return readScalar<std::uint16_t>(Address, absolute); }

	static inline std::uint32_t ReadInt(std::uint64_t Address, bool absolute = false) { return readScalar<std::uint32_t>(Address, absolute); }

	static inline std::uint64_t ReadLong(std::uint64_t Address, bool absolute = false) { return readScalar<std::uint64_t>(Address, absolute); }

	static inline float ReadFloat(std::uint64_t Address, bool absolute = false) { return readScalar<float>(Address, absolute); }

	static inline bool ReadBool(std::uint64_t Address, bool absolute = false) { return ReadByte(Address, absolute) != 0; }
	...
	template <typename T, std::enable_if_t<std::is_trivially_copy_assignable_v<T>, int> = 0> static void writeScalarAbsolute(std::uint64_t Address, T t) {
		protect_lock lock(Address, sizeof(T));
		if (!lock.good())
			return;
	...
	static inline void WriteByteAbsolute(std::uint64_t Address, std::uint8_t Input) { writeScalarAbsolute<std::uint8_t>(Address, Input); }

	static inline void WriteShortAbsolute(std::uint64_t Address, std::uint16_t Input) { writeScalarAbsolute<std::uint16_t>(Address, Input); }

	static inline void WriteIntAbsolute(std::uint64_t Address, std::uint32_t Input) { writeScalarAbsolute<std::uint32_t>(Address, Input); }

	static inline void WriteLongAbsolute(std::uint64_t Address, std::uint64_t Input) { writeScalarAbsolute<std::uint64_t>(Address, Input); }

	static inline void WriteFloatAbsolute(std::uint64_t Address, float Input) { writeScalarAbsolute<float>(Address, Input); }

	static inline void WriteBoolAbsolute(std::uint64_t Address, bool Input) { WriteByteAbsolute(Address, Input ? 1 : 0); }
	...
	template <typename T> static void writeScalar(std::uint64_t Address, T const &t, bool absolute = false) {
		if (absolute) {
	...
	static inline void WriteByte(std::uint64_t Address, std::uint8_t Input, bool absolute = false) { writeScalar<std::uint8_t>(Address, Input, absolute); }

	static inline void WriteShort(std::uint64_t Address, std::uint16_t Input, bool absolute = false) { writeScalar<std::uint16_t>(Address, Input, absolute); }

	static inline void WriteInt(std::uint64_t Address, std::uint32_t Input, bool absolute = false) { writeScalar<std::uint32_t>(Address, Input, absolute); }

	static inline void WriteLong(std::uint64_t Address, std::uint64_t Input, bool absolute = false) { writeScalar<std::uint64_t>(Address, Input, absolute); }

	static inline void WriteFloat(std::uint64_t Address, float Input, bool absolute = false) { writeScalar<float>(Address, Input, absolute); }

	static inline void WriteBool(std::uint64_t Address, bool Input, bool absolute = false) { WriteByte(Address, Input ? 1 : 0, absolute); }
